...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
    <div>
        <h1 class="h4 mb-0">Formatos de plaqueteo</h1>
        <p class="text-muted small mb-0">Registra la evidencia de cada plaqueteo realizado por la Alcaldía.</p>
    </div>
    <a href="<?= Url::to('/formatos-plaqueteo/historial') ?>" class="btn btn-sm btn-outline-secondary">Ver histórico</a>
</div>
...
<?php if (Auth::esSuperusuario()): ?>
    ...
    document.getElementById('selectorInstitucion').addEventListener('change', function () {
        window.location = <?= json_encode(Url::to('/formatos-plaqueteo')) ?> + (this.value ? '?institucion=' + encodeURIComponent(this.value) : '');
    });
    </script>
<?php endif; ?>
